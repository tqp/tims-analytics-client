...
export class CrudMasterServerPaginationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private calculatePageSize(): number {
    ...
    // console.log('calc pageSize', this.pageSize);
  ...
  private setInitialFieldValues(): void {
    ...
    this.searchParams.pageSize = this.calculatePageSize();
  ...
  private getPage(searchParams: ServerSidePaginationRequest) {
    ...
    this.crudService.getPersonList_InfiniteScroll(searchParams).subscribe((response: ServerSidePaginationResponse) => {
        ...
        this.pageEnd = this.pageStart + this.paginator.pageSize - 1;
